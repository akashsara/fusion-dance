...
experiment_name = f"fusion_dual_input_autoencoder_v1"
...
with torch.no_grad():
    ...
    epoch_fusee = model(sample_fusee, sample_fusee)
    epoch_fusion = model(sample_base, sample_fusee)
...
with torch.no_grad():
    ...
    # Normal Image
    output_samples = model(test_sample, test_sample).detach().cpu()
    # Fusion Image
    output_base = model(test_sample_base, test_sample_base).detach().cpu()
    output_fusee = model(test_sample_fusee, test_sample_fusee).detach().cpu()
    output_fusion = model(test_sample_base, test_sample_fusee).detach().cpu()
