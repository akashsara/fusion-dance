...
experiment_name = f"finetuned_fusion_cnn_prior_v1"
...
discriminator_experiment_name = f"fusion_discriminator_v1"
discriminator_input_channels = 3
discriminator_input_dim = image_size
discriminator_num_filters = 512
discriminator_num_layers = 4

prior_experiment_name = f"fusion_cnn_prior_v2"
...
discriminator_model_prefix = f"outputs\\{discriminator_experiment_name}"
prior_model_prefix = f"outputs\\{prior_experiment_name}"

vq_vae_model_path = os.path.join(vq_vae_model_prefix, "model.pt")
discriminator_model_path = os.path.join(discriminator_model_prefix, "model.pt")
prior_model_path = os.path.join(prior_model_prefix, "model.pt")
...
for epoch in range(epochs):
    ...
    for iteration, batch in enumerate(tqdm(train_dataloader)):
        ...
        if mode == "discrete":
            image = y_hat.argmax(dim=1).flatten(start_dim=1).view(-1, 1)
            image = vq_vae.quantize_and_decode(image, target_shape, device)
        elif mode == "continuous":
            image = vq_vae.decoder(y_hat)
        elif mode == "continuous-final_image":
            image = y_hat
        discriminator_out = discriminator(image).sigmoid()

        # Calculate Loss
        discriminator_loss = discriminator_criterion(
            discriminator_out, torch.ones_like(discriminator_out, device=device)
        )
        batch_loss = discriminator_loss
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(tqdm(val_dataloader)):
            ...
            # Calculate Loss
            discriminator_loss = discriminator_criterion(
                discriminator_out, torch.ones_like(discriminator_out)
            )
            batch_loss = discriminator_loss
            if use_standard_loss:
                batch_loss += criterion(y_hat, y)
